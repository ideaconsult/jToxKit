...
describe("jToxKit Core", function () {
  ...
  describe("Raw Solr Translation", function (){
    var Translation = a$(jT.RawSolrTranslation);
    var topic = new Translation();
    
    it("Can translate data", function () {
      var data = topic.translateResponse(RawSolrData);
...
// Data files. TODO: Bonus if anybody can find a (feasible) way to put this into an external file
// and be able to load it from BOTH npm and the browser.
var RawSolrData = {
    "responseHeader": {
        "zkConnected": true,
        "status": 0,
